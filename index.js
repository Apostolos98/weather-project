// to avoid creating multiple times 
let elementsHaveBeenCreated = false;
let toggleCF = 'celcius';
// for displaying the  city 
let city; 
// data the code needs
// beeing global helps for dropdown to display correct info through multiple searches
let hourlyDetails = [];
// icons highlighted for each day 
let selectedIcon1;
let selectedIcon2;
let selectedIcon3;
// to avoid adding multiple listeners to the same object
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function main(){
        ...
        // else nothing will happend and user must enter a location
    }
...
function findDay(dat){
    const date = new Date(dat);
    const daysOfWeek = ['Sunday', 'Monday', 'Tuesday', 'Wednesday','Thursday', 'Friday', 'Saturday']
    return daysOfWeek[date.getDay()];
}

// writing corresponding info to dropdown
...
function highlight(){
    ...
    document.querySelectorAll('.hour-element').forEach((el, ind) => {
        ...
        // to have the closest to users hour icon-temp highlighted and coresponding col3 info 
        // when elements have first been created or when a new city is searched
        const time = new Date();
        const timeHour = time.getHours();
        if ((el.querySelector('h3').innerHTML.slice(0, 2) == timeHour || 
             el.querySelector('h3').innerHTML.slice(0, 2) == timeHour - 1)) {
                if (ind < 12){
                    // the if != undefined is so that if a new city is searched and an hour-icon other than users current hour is highlighted this code will 
...
// for changing units
// this function must only be after display so there is content inside
function setCelFahr(){
...
// for the dislay5 to display units according to toggleCF variable
...
function createElements(container, numOfDays=3){
    ...
    for (let indexOfDay = 0;indexOfDay < numOfDays; indexOfDay++) {
        ...
        frag.appendChild(leftColumn)
        frag.appendChild(rightColumn)
